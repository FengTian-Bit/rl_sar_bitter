...
RL_Sim::RL_Sim()
{
    ...
    for (int i = 0; i < this->params.num_of_dofs; ++i)
    {
        ...
        this->joint_subscribers[joint_name] =
            ...
            ); //lambda表达式捕获关节名称,调用JointStatesCallback更新对应关节的状态
    ...
    std::cout << LOGGER::INFO << "Please press Enter to start control robot" << std::endl;
}
...
void RL_Sim::GetState(RobotState<double> *state)
{
    ...
    else if (this->params.framework == "isaacsim")
    {
        ...
        //           << "pose.orientation.z: " << std::fixed << std::setprecision(5) << this->pose.orientation.z << std::endl;
        //从rosbag中获取，即读取/BitterImu话题的值
    ...
    for (int i = 0; i < this->params.num_of_dofs; ++i)
    {
        ...
        //           << std::fixed << std::setprecision(5) << state->motor_state.tau_est[i] << std::endl;
...
void RL_Sim::SetCommand(const RobotCommand<double> *command)
{
    ...
    for (int i = 0; i < this->params.num_of_dofs; ++i)
    {
        ...
        //           << std::fixed << std::setprecision(5) << this->joint_publishers_commands[i].tau << std::endl;
        // //把车轮设为0
        // if (i == 3 || i == 7 || i == 11 || i == 15){
        //     this->joint_publishers_commands[i].q = 0;
