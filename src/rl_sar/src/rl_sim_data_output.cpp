...
RL_Sim::RL_Sim()
{
    this->is_simulation = true;
     ...
     this->log_file.open("/home/bit/rl_sar_wjj/log.txt", std::ios::out | std::ios::app);
...
void RL_Sim::RobotControl()
{
    if (this->control.control_state == STATE_RESET_SIMULATION)
    {
        ...
        this->control.control_state = this->control.last_control_state;//将状态恢复为上一个控制状态
    }
    if (this->control.control_state == STATE_TOGGLE_SIMULATION)
    {
        std_srvs::Empty empty;//创建一个empty消息，用于调用服务
    ...
    if (simulation_running)
    {
        ...
        // std::cout << LOGGER::INFO << "Motion Time: " << this->motiontime << std::endl;
        // std::cout << LOGGER::INFO << "Command Size: " << this->robot_command.motor_command.q.size() << std::endl;
        // for (int i = 0; i < this->params.num_of_dofs; ++i)
        // {
        //     std::cout << LOGGER::INFO << "Joint " << i << " - Position: " << this->joint_positions[this->params.joint_controller_names[i]]
        //               << ", Velocity: " << this->joint_velocities[this->params.joint_controller_names[i]]
        //               << ", Effort: " << this->joint_efforts[this->params.joint_controller_names[i]] << std::endl;
        // }

        this->log_file << "Motion Time: " << this->motiontime << std::endl;
        this->log_file << "Command Size: " << this->robot_command.motor_command.q.size() << std::endl;
        for (int i = 0; i < this->params.num_of_dofs; ++i) {
            this->log_file << "Joint " << i << " - Position: " << this->joint_positions[this->params.joint_controller_names[i]]
                           << ", Velocity: " << this->joint_velocities[this->params.joint_controller_names[i]]
                           << ", Effort: " << this->joint_efforts[this->params.joint_controller_names[i]] << std::endl;
        }
    }
...
void RL_Sim::RunModel()
{
    if (this->rl_init_done && simulation_running)
    {
        ...
        // std::cout << LOGGER::INFO << "Episode Length: " << this->episode_length_buf << std::endl;
        // std::cout << LOGGER::INFO << "Command Size: " << this->obs.commands.size(0) << std::endl;
        // for (int i = 0; i < this->params.num_of_dofs; ++i)
        // {
        //     std::cout << LOGGER::INFO << "Joint " << i << " - Position: " << this->robot_state.motor_state.q[i]
        //               << ", Velocity: " << this->robot_state.motor_state.dq[i]
        //               << ", Effort: " << this->robot_state.motor_state.tau_est[i] << std::endl;
        // }

        // 输出到日志文件
        this->log_file << "Episode Length: " << this->episode_length_buf << std::endl;
        this->log_file << "Command Size: " << this->obs.commands.size(0) << std::endl;
        for (int i = 0; i < this->params.num_of_dofs; ++i) {
            this->log_file << "Joint " << i << " - Position: " << this->robot_state.motor_state.q[i]
                           << ", Velocity: " << this->robot_state.motor_state.dq[i]
                           << ", Effort: " << this->robot_state.motor_state.tau_est[i] << std::endl;
        }
